  ...
  useEffect(() => {
    fetch('/api/cloudinary/status')
      .then(async response => {
        ...
        if (!response.ok) {
          throw new Error(data?.message || 'Status Cloudinary belum dapat diperiksa.');
        }
        setCloudinaryStatus(data as CloudinaryStatus);
      })
      .catch(() => setCloudinaryStatus({
        configured: false,
        message: 'Status Cloudinary belum dapat diperiksa.',
      }));
  }, []);
